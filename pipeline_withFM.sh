...
Step 3-a. Registration of T1 to B0 when field map is available
The following cells runs the BBR of T1 to B0 when field map is available. If not, jump to Step3-b

echo  " **** Calculating matrix to move T1 to B0 using BBR ****
	        -------------------------------------------------------"
for i in `cat subjects.txt`;
do epi_reg --fmapmag=${workdir}/"$i"/T1/gre_field_mapping.nii  -- epi=${workdir}/"$i"/boundary_mask/nodif.nii.gz  --t1=${workdir}/"$i"/boundary_mask/MASK/T1.nii.gz  --t1brain=${workdir}/"$i"/boundary_mask/MASK/T1_brain.nii.gz  --pedir=-y --out=${workdir}/"$i"/boundary_mask/transform/epi_reg;
convert_xfm -omat ${workdir}/"$i"/boundary_mask/transform/T1_to_nodif_bbr.mat -inverse ${workdir}/"$i"/boundary_mask/transform/epi_reg.mat
